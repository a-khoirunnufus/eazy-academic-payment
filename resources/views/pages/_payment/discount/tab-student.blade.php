<table id="table-discount-receiver-student" class="table table-striped">
    <thead>
        <tr>
            <th>
...
<script>
    $(function() {
        _discountReceiverStudentTable.init();

        $('#table-discount-receiver-student #check-all-receiver').on('change', function() {
            if (this.checked) {
                $('#table-discount-receiver-student input.check-receiver').each(function() {
    ...
    const _discountReceiverStudentTable = {
        ..._datatable,
        init: function() {
            this.instance = $('#table-discount-receiver-student').DataTable({
                ajax: {
                ...
                initComplete: function() {
                    $('.custom-actions').html(`
                        <div style="margin-bottom: 7px">
                            <button onclick="_discountReceiverStudentTableActions.add()" class="btn btn-info">
        ...
        template: {
            rowAction: function(row) {
                ...
                if(row.mdr_status_generate === 0){
                    action = `<a onclick="_discountReceiverStudentTableActions.edit(this)" class="dropdown-item" href="javascript:void(0);"><i data-feather="edit"></i>&nbsp;&nbsp;Edit</a>
                    <a onclick="_discountReceiverStudentTableActions.delete(this)" class="dropdown-item" href="javascript:void(0);"><i data-feather="trash"></i>&nbsp;&nbsp;Delete</a>`;
                }
                return `
    ...
    const _discountReceiverStudentTableActions = {
        add: function() {
            Modal.show({
                type: 'form',
        ...
        edit: function(e) {
            let data = _discountReceiverStudentTable.getRowData(e);
            Modal.show({
                type: 'form',
            ...
            _discountReceiverStudentTable.selected = data
        },
        delete: function(e) {
            let data = _discountReceiverStudentTable.getRowData(e);
            Swal.fire({
    ...
    const copyStudentReceiverActions = {
        setupElements: () => {
            ...
            $('#modal-copy-data-student select#select-all-period').on('change', function() {
                ...
                $('#form-copy-data-student select[name="mdr_period[]"]').each(function() {
            ...
            $('#modal-copy-data-student input.input-all-nominal').on('change', function() {
                ...
                $('#form-copy-data-student input.input_mdr_nominal').each(function() {
            ...
            $('#modal-copy-data-student select#select-all-status').on('change', function() {
                const selectedValue = $(this).val();
                $('#form-copy-data-student select[name="mdr_status[]"]').each(function() {
                    $(this).val(selectedValue);
                });
            });
        },
        openModalCopyData: async () => {
            ...
            $('#table-discount-receiver-student input.check-receiver:checked').each(function() {
                const rowIdx = $(this).data('dt-row');
                const row = _discountReceiverStudentTable.instance.row(parseInt(rowIdx)).data();
                        ...
                        <td>
                            <div>
                                <span class="fw-bold text-nowrap">
                                    ...
                                    <a class="btn d-inline-block p-0" onclick="copyStudentReceiverActions.showDiscountDetailModal(${row.discount.md_id})"><i data-feather="info"></i></a>
                                </span>
                            </div>
                            <input type="hidden" name="md_id[]" value="${row.md_id}" />
                        </td>
                        <td>
                            <select name="mdr_period[]" class="form-select form-select-sm w-150" value="${row.mdr_period}">
                                ${
                                    schoolYear
                                        .filter(item => {
        ...
        validateData: () => {
            return new Promise(async (resolve, reject) => {
                ...
                const res = await $.ajax({
                    async: true,
                    url: _baseURL + '/api/payment/discount-receiver/validate-batch',
                    type: 'post',
                    data: data,
                });
        ...
        storeBatch: async () => {
            ...
            try {
                ...
                const res = await $.ajax({
                    async: true,
                    url: _baseURL + '/api/payment/discount-receiver/store-batch',
                    type: 'post',
                    data: formData,
                    contentType: false,
                    processData: false,
